...
#define VEN_PIN             	(58 | 0x80000000)
#define GPIO4_PIN       	(42 | 0x80000000)
#define IRQ_PIN         	(57 | 0x80000000)
//#define CLK_REQ_PIN             (69 | 0x80000000) /* add by binpeng.huang on 2016.05.29 for Defect 2201390 */

#define EINT_NUM                1
#define PN544_I2C_GROUP_ID	1
#define PN544_WRITE_ID		0x28
...
{	
    pr_err("nfc test: nfc show function!\n");

    if (!dev ) {
       
        return 0;
    }
    pr_err("The nfc is pn547 \n");
...
//	struct device * nfc_device;
	...
	// write data, note use dma buffer to transfer, or change the i2c dma flag, if not, will report error
	 ...
	 {
		 pr_err("pn544 %s : no pn544 or it is bad %d\n", __func__, ret);
		 nfc_exist=0;
		 return -1;
	 }
	 else
	 {
		 pr_err("pn544 %s :pn544 test OK %d\n", __func__, ret);
		 nfc_exist=1;
		 return 0;
	 }
}
...
//	char tmp[MAX_BUFFER_SIZE];
	...
	pr_err("pn544 %s : reading %zu bytes.\n", __func__, count);

	mutex_lock(&pn544_dev->read_mutex);

	if (!mt_get_gpio_in(IRQ_PIN)) {
        
		pr_err("pn544 read no event\n");
		...
		if (ret) {
			pr_err("pn544 read wait event error\n");
			goto fail;
		}
	}
...
static ssize_t pn544_dev_write(struct file *filp, const char __user *buf, size_t count, loff_t *offset)
{
	...
	pr_err("pn544 %s : writing %zu bytes.\n", __func__, count);
	/* Write data */
	...
	switch (cmd) {
		case PN544_SET_PWR:
			if (arg == 2) {
				...
				pr_err("pn544 %s power on with firmware\n", __func__);
				mt_set_gpio_out(VEN_PIN, 0);
			pr_err(" ppn544_dev_ioctl_VEN_PIN=%d\n",mt_get_gpio_out(VEN_PIN));
				mt_set_gpio_out(GPIO4_PIN, 1);
			pr_err(" ppn544_dev_ioctl_GPIO4_PIN=%d\n",mt_get_gpio_out(GPIO4_PIN));
				msleep(10);
			...
			pr_err(" ppn544_dev_ioctl_VEN_PIN=%d\n",mt_get_gpio_out(VEN_PIN));
				...
				pr_err("pn544 %s power off\n", __func__);
				mt_set_gpio_out(GPIO4_PIN, 0);
				mt_set_gpio_out(VEN_PIN, 1);
				msleep(10);
			} else {
				pr_err("pn544 %s bad arg %lu\n", __func__, arg);
				return -EINVAL;
			}
			break;
		default:
			pr_err("pn544 %s bad ioctl %u\n", __func__, cmd);
			return -EINVAL;
	}
...
static int pn544_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pr_err("pn544 nfc probe step01 is ok\n");
	...
	pr_err("pn544 nfc probe step02 is ok\n");
	...
	pr_err("pn544 nfc probe step03 is ok\n");
	...
	pr_err("pn544 nfc probe step04 is ok\n");
#ifdef CONFIG_64BIT
	...
	if (I2CDMAReadBuf == NULL) 
	{
        ...
        pr_err(KERN_DEBUG "%s :I2CDMAWriteBuf_pa %x, I2CDMAReadBuf_pa,%x\n", __func__, I2CDMAWriteBuf_pa, I2CDMAReadBuf_pa);
	...
	pr_err("pn544 nfc probe step05 is ok\n");
    ...
    if (node)
    {
	    pr_err("pn544 find irq-nfc node\n");
	    ...
	    {
		    ret = -1;
		    pr_err("pn544 register clk irq error\n");
	    }
    }
    else
    {
		pr_err("%s : can not find pn547-irq-nfc node\n",  __func__);
    }
	pn544_disable_clk_irq(pn544_dev);
	/* end: add by binpeng.huang.hz@tcl.com on 2016.05.29 for Defect 2201390 */
	...
	pr_err("pn544 nfc probe step06 is ok\n");
	...
	pr_err("pn544 nfc probe step07 is ok\n");
...
{
         int ret ;
	 pr_err("add pn544_driver\n");
	 
	 ret = i2c_add_driver(&pn544_driver);
	 if(ret !=0)
        {
            pr_err("add pn544_driver fail\n");
            return -1;
        }	
	 return 0;
}
...
static int __init pn544_dev_init(void)
{
	...
	msleep(10);
	if( ret){
		pr_err("platform_device_register pn544 failed!\n");
		return ret;
	}
...
{
	pr_err("pn544 Unloading pn544 driver\n");
