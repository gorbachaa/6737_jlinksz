...
static ssize_t dev_char_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	if (skb) {
		...
		if (ret){
			CCCI_NORMAL_LOG(port->md_id, CHAR, "write error done 1\n");
			goto err_out;
		}
		if (port->flags & PORT_F_USER_HEADER) {
		...
		if (port->rx_ch == CCCI_IPC_RX) {
			...
			if (ret < 0){
				CCCI_NORMAL_LOG(port->md_id, CHAR, "write error done 2\n");
				goto err_out;
			}
			else
		...
		if (ret) {
			if (ret == -EBUSY && !blocking)
				ret = -EAGAIN;
			CCCI_NORMAL_LOG(port->md_id, CHAR, "write error done 3\n");
			goto err_out;
		}
		return actual_count;
