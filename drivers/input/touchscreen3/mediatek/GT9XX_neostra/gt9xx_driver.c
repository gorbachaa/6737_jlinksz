...
//#include "./../../../misc/mediatek/hdmi/hdmitx/mt8163/inc/hdmicmd.h"
...
#ifdef CONFIG_GTP_CHARGER_DETECT
u32 chr_status_last = 0;
#define TPD_CHARGER_CHECK_CIRCLE		400
...
reset_proc:
	...
	GTP_ERROR("turn on power reg-vgp1\n");
...
static u8 gtp_bak_ref_proc(struct i2c_client *client, u8 mode)
{
		...
		if (IS_ERR(flp)) {
			...
			flp->f_op->llseek(flp, 0, SEEK_SET);
			//huangjialong mod
			ret = vfs_read(flp, (char *)refp, ref_len, &flp->f_pos);
		...
		if (GTP_BAK_REF_STORE == mode) {
			...
			flp->f_op->llseek(flp, 0, SEEK_SET);
			//flp->f_op->write(flp, (char *)refp, ref_len, &flp->f_pos);
			vfs_write(flp, (char *)refp, ref_len, &flp->f_pos);
		} else {
			/* checksum ref file */
			for (j = 0; j < ref_grps; ++j) {
				ref_chksum = 0;
...
static u8 gtp_main_clk_proc(struct i2c_client *client)
{
		...
		if (gup_check_fs_mounted("/data") == FAIL) {
			...
			ret = vfs_read(flp, (char *)gtp_clk_buf, 6, &flp->f_pos);
				if (ret > 0) {
					ret = gtp_check_clk_legality();
					if (SUCCESS == ret) {
		...
		if (IS_ERR(flp)) {
			...
			vfs_write(flp, (char *)gtp_clk_buf, 6, &flp->f_pos);
		}
...
//Begin Neostra liudongke add to read GTP version's interface 20170727
		...
		struct device_attribute *attr, char *buf)
{
    ...
    if (ret < 0)
    {
        GTP_ERROR("Read Firmware version failed.");
        ret = -EINVAL;
        return ret;
    }

	ret = i2c_read_bytes(i2c_client_point, GTP_REG_CONFIG_DATA, &flash_cfg_version, 1);
	if (ret < 0)
    {
        GTP_ERROR("Read CFG version failed.");
        ret = -EINVAL;
        return ret;
    }

    //Begin Neostra liudongke add to read GTP vendor id 20170727
	ret = i2c_read_bytes(i2c_client_point, GTP_REG_SENSOR_ID, &sensor_id, 1);	
	if (ret < 0)
    {
        GTP_ERROR("Read VENDER SENSOR ID failed.");
        ret = -EINVAL;
        return ret;
    }	
	//GTP_ERROR("ldk Read TP cfg version=%d\n",flash_cfg_version);	
	//GTP_ERROR("ldk Read TP Firmware version=%02x\n",version_info);
	//GTP_ERROR("ldk Read TP VENDER SENSOR ID=%d\n",sensor_id);	
	return scnprintf(buf, PAGE_SIZE, "ID:%d Goodix FW:%02x & CFG:%d\n", sensor_id,version_info,flash_cfg_version);	
...
//End Neostra liudongke add to read GTP version's interface 20170727
...
//		tpd_flag = 1;
	...
	touch_sysfs_init();//Neostra liudongke add to read GTP version's interface 20170727
#ifdef CONFIG_NEOSTRA_HW_INFO_NODE		
    neostra_add_hw_info("tp-ic", "gt910", 0);	
	neostra_add_hw_info("tp-fw", NULL, version_info);
	neostra_add_hw_info("tp-vendor", NULL, 0);
#endif	
...
#endif
    touch_sysfs_deinit();//Neostra liudongke add to read GTP version's interface 20170727
	gtp_free_gpio_res();
	return 0;
}
...
if(hdmi_isenable == hdmi_isenable_last)
	{	
		}
	else{
if(hdmi_isenable){
	msleep(200);
			...
			hdmi_isenable_last=hdmi_isenable;
	}else{
			ret = gtp_i2c_write(i2c_client_point, chr_cmd, 3);
			if (ret > 0) {
